...
class MdblistProcessor:
    """
    Class for processing MDBList collections and managing Emby collections.
    """
    
    def __init__(self):
        """Initialize the MdblistProcessor with service connections and counters."""
        # Counters and collection tracking
        self.newly_added = 0
        self.newly_removed = 0
        self.collection_ids_with_custom_sorting: List[str] = []
        self.all_collections_ids: List[str] = []
        
        # Initialize service connections
        self.emby = Emby(EMBY_SERVER_URL, EMBY_USER_ID, EMBY_API_KEY)
        self.mdblist = Mdblist(MDBLIST_API_KEY)
        self.item_sorting = ItemSorting(self.emby)
        self.refresher = Refresher(self.emby)
        self.db_manager = Db()
        
        # Load collections configuration
        self.collections_parser = load_config()
    ...
    def set_poster(self, collection_id: str, collection_name: str, poster_path: Optional[str] = None) -> bool:
        ...
        current_poster = self.db_manager.get_config_for_section(collection_id, "poster_path")
        ...
        try:
            if self.emby.set_image(collection_id, poster_path):
                self.db_manager.set_config_for_section(collection_id, "poster_path", poster_path)
    ...
    def process_hardcoded_lists(self) -> None:
        """
        ...
        for section in self.collections_parser.sections():
            try:
                # Create a dictionary of collection properties
                collection_config = {
                    "name": section,
                    "id": self.collections_parser.get(section, "id", fallback=None),
                    "source": self.collections_parser.get(section, "source", fallback=""),
                    "poster": self.collections_parser.get(section, "poster", fallback=None),
                    "frequency": self.collections_parser.get(section, "frequency", fallback=100),
                    "mdblist_name": self.collections_parser.get(section, "list_name", fallback=None),
                    "user_name": self.collections_parser.get(section, "user_name", fallback=None),
                    "update_items_sort_names": self.collections_parser.getboolean(
                        section, "update_items_sort_names", fallback=False
                    ),
                    "collection_sort_name": self.collections_parser.get(
                        section, "collection_sort_name", fallback=None
                    ),
                    "collection_sort_prefix": self.collections_parser.get(
                        section, "collection_sort_prefix", fallback=None
                    ),
                    "collection_sort_date": self.collections_parser.getboolean(
                        section, "collection_sort_date", fallback=False
                    ),
                    "overwrite_description": self.collections_parser.get(
                        section, "description", fallback=None
                    ),
                    "active_between": self.collections_parser.get(
        ...
        for mdblist_list in collections:
            try:
                self.process_list(mdblist_list)
            except Exception as e:
                logger.error(f"Error processing list '{mdblist_list.get('name', 'unknown')}': {e}")
    
    def process_my_lists_on_mdblist(self) -> None:
        """
        ...
        try:
            ...
            my_lists = self.mdblist.get_my_lists()
    ...
    def process_list(self, mdblist_list: Dict[str, Any]) -> None:
        """
        ...
        collection_id = self.emby.get_collection_id(collection_name)
        ...
        if active_period_str:
            if not inside_period(active_period_str):
                ...
                try:
                    ...
                    all_items_in_collection = self.emby.get_items_in_collection(collection_id, ["Id"])
                    item_ids = (
                    ...
                    if item_ids:
                        self.newly_removed += self.emby.delete_from_collection(collection_name, item_ids)
                        logger.info(f"Collection '{collection_name}' is not active. Removed {self.newly_removed} items.")
                    else:
        ...
        try:
            ...
            if list_id is not None:
                mdblist_imdb_ids, mdblist_mediatypes = self.mdblist.get_list(list_id)
            # Get items based on list name and user name
            elif mdblist_name is not None and user_name is not None:
                found_list_id = self.mdblist.find_list_id_by_name_and_user(mdblist_name, user_name)
                if found_list_id is None:
                    logger.error(f"List '{mdblist_name}' by '{user_name}' not found. Skipping.")
                    return
                mdblist_imdb_ids, mdblist_mediatypes = self.mdblist.get_list(found_list_id)
            # Get items based on source URL(s)
                ...
                for url in sources:
                    imdb_ids, mediatypes = self.mdblist.get_list_using_url(url.strip())
                    if imdb_ids:
        ...
        if collection_id is None:
            ...
            try:
                collection_items = self.emby.get_items_in_collection(collection_id, ["ProviderIds"])
        ...
        add_emby_ids = self.emby.get_items_with_imdb_id(missing_imdb_ids, mdblist_mediatypes)
        ...
        if collection_id is None:
            ...
            try:
                collection_id = self.emby.create_collection(collection_name, [add_emby_ids[0]])
                add_emby_ids.pop(0)
        ...
        if collection_id not in self.all_collections_ids:
            self.all_collections_ids.append(collection_id)
        
        if update_collection_items_sort_names and collection_id not in self.collection_ids_with_custom_sorting:
            self.collection_ids_with_custom_sorting.append(collection_id)
        
        # Add and remove items
        try:
            items_added = self.emby.add_to_collection(collection_name, add_emby_ids)
            self.newly_added += items_added
            
            items_removed = self.emby.delete_from_collection(collection_name, remove_emby_ids)
            self.newly_removed += items_removed
        ...
        self.set_poster(collection_id, collection_name, poster)
        ...
        try:
            ...
            if sort_title_update:
                self.emby.set_item_property(collection_id, "ForcedSortName", collection_sort_name)
            else:
        ...
        try:
            ...
            if (USE_MDB_COLLECTION_DESCRIPTION and 
                ...
                description = self.strip_quotes(description)
                self.emby.set_item_property(collection_id, "Overview", description)
            ...
            # Use overwrite description if specified
                ...
                # Strip any leading and trailing quotes from the description
                overwrite_description = self.strip_quotes(overwrite_description)
                self.emby.set_item_property(collection_id, "Overview", overwrite_description)
